...
function OrbitItem({ item, index, totalItems, path, itemSize, rotation, progress, fill }: OrbitItemProps) {
  ...
  // Compute each item's effective position (0–100) along the orbit path
  const effectiveOffset = useTransform(progress, (p: number) => {
    return (((p + itemOffset) % 100) + 100) % 100;
  });

  const offsetDistance = useTransform(effectiveOffset, (o: number) => `${o}%`);

  // Front arc (50–100%) renders above the center logo (z-10); back arc (0–50%) renders behind it
  const zIndex = useTransform(effectiveOffset, (o: number) => (o > 50 ? 20 : 5));
