...
function MetricsStat({ label, value, delta, up }: { label: string; value: string; delta: string; up: boolean }) {
  return (
...
function LogStream() {
  ...
  const col: Record<string, string> = { INFO: "text-sky-400", WARN: "text-amber-400", ERROR: "text-red-400" };
...
export default function FeaturesStack() {
  return (
    <section className="w-full max-w-7xl mx-auto px-6 md:px-12 py-24">
      ...
      {/* ── The Giant Fabrica-Frame Showcase Card ── */}
      <div className="w-full min-h-[70vh] md:min-h-[80vh] rounded-[32px] border border-zinc-900 bg-zinc-950/40 p-8 md:p-16 relative overflow-hidden flex flex-col justify-between group">

        {/* Bronze ambient hover glow — top-left sweep */}
          ...
          <div className="md:col-span-3 flex flex-col justify-end gap-6 h-full">
            <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-5 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-4">
                ...
                <MetricsStat label="Uptime"      value="99.99%" delta="30 day avg" up={true} />
              </div>
            </div>
            ...
            <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-5 space-y-2">
              ...
              {[
                ...
                { name: "Neon DB",      status: "Healthy" },
                { name: "Upstash",      status: "Healthy" },
                { name: "Vercel Edge",  status: "Healthy" },
              ].map((s) => (
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-3 text-[10px] text-zinc-600">
                  <span className="flex items-center gap-1">
                    <span className="w-2 h-0.5 bg-white/20 inline-block rounded" />Requests
                  </span>
                  <span className="flex items-center gap-1">
